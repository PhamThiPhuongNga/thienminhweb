...
var fmLib =  {
  ...
  inputFocus: false,
						...
						if(fmEntries[curCont][id].isDir) {
							...
							if(typeof fmContSettings[curCont].customAction == 'object' && fmContSettings[curCont].customAction.action) {
								items.push({icon:'cursor.png',caption:fmContSettings[curCont].customAction.caption,exec:[fmContSettings[curCont].customAction.action,curCont,id,fmEntries[curCont][id].fullName]});
                                                        ...
                                                        // bo xung them tinh nang chon, lay link anh sang editor	 senFileToEditor							
							items.push({icon:'editor.png',caption:fmMsg[lang].cmdToEditor,exec:['fmLib.senUrlToEditor',fmEntries[curCont].listJson.path,fmEntries[curCont][id].fullName]});
	...
	senUrlToEditor: function(path,img){	
		var funcNum = getUrlParam('CKEditorFuncNum'); 
		var choiceImg = 0,id;
		var type,txtHide,imageURL;		
		if(getUrlParam('choiceImg')!=null){
			choiceImg = getUrlParam('choiceImg');
		}
		...
		if(getUrlParam('txtHide')!=null){
			txtHide = getUrlParam('txtHide');
		}
		imageURL = mediaUrl + path+"/"+img;			
		if(choiceImg==1){	
			var obj 		= new Object();				
			obj.id 			= id;
			obj.type 		= type;
			...
			window.opener.fcall.fcGetUrlChoiceImg(obj);
		}else{
