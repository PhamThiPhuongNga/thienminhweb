...
class Model
{
	...
	static function printMenuAction($table, $parent_id, $str, $taxonomy)
	{
		...
		foreach ($term_taxonomy as $news_topic) {
			if ($news_topic->submenu > 0) {
?>
				<tr class="<?php echo ($parent_id == 0) ? 'bg-gray-light' : ''; ?> valign-middle">
					<td><?php echo $str . $news_topic->title_vi ?></td>
					<td><?php echo $str . $news_topic->title_en ?></td>
					<td><?php echo $news_topic->url_part ?></td>
					<td><?php echo $news_topic->count ?></td>
					<td>
						<select class="form-control" name="position" id="position" onchange="update_value_by_id('<?php echo $table ?>', 'position', '<?php echo $news_topic->id ?>', this.value);">
							<option value="">Chọn vị trí</option>
							<option value="header" <?php echo ($news_topic->position == 'header') ? 'selected="selected"' : '' ?>>Đầu trang</option>
							<option value="home" <?php echo ($news_topic->position == 'home') ? 'selected="selected"' : '' ?>>Trang chủ</option>
							<option value="left" <?php echo ($news_topic->position == 'left') ? 'selected="selected"' : '' ?>>Menu trái</option>
							<option value="right" <?php echo ($news_topic->position == 'right') ? 'selected="selected"' : '' ?>>Menu phải</option>
							<option value="footer" <?php echo ($news_topic->position == 'footer') ? 'selected="selected"' : '' ?>>Cuối trang</option>
						</select>
					</td>
					<td>
			...
			?>
				<tr class="<?php echo ($parent_id == 0) ? 'bg-gray-light' : ''; ?> valign-middle">
					...
					<td><?= Model::news_topic_view_link($news_topic) ?></td>
				</tr>
<?php
			}
		}
