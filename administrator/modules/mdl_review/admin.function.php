...
class Model
{
	static function web_image_view_link($row)
	{
		...
		$str .= '<a href="javascript:void(0);" data-toggle="modal" data-target="#showCNTT"  onclick="showCNTT(\'' . $row->id . '\',\'ajax/review/ajax.web_image_get.php\')"><i class="fa fa-pencil-square-o" data-toggle="tooltip" title="Cập nhật thông tin"></i></a>&nbsp;&nbsp;';
		$str .= '<a href ="?module=review&task=web_image_delete&id=' . $row->id . '" onclick="return confirmAction();"><i class="fa fa-trash text-red" data-toggle="tooltip"  title="Xóa"></i></a>&nbsp;&nbsp;';
	...
	{
		global $database;
		$query = "SELECT a.* FROM e4_review a ORDER BY  a.id desc ";
	...
	static function web_image_add()
	{
		...
		if ($_POST["submit"] == "web_image_add") {
			...
			if ($database->insertObject('e4_review', $row, 'id'))
	...
	static function web_image_edit()
	{
		...
		if ($_POST["submit"] == "web_image_edit") {
			...
			if ($database->updateObject('e4_review', $row, 'id'))
				$ariacms->redirect("", "javascript:history.back()");
			else echo $database->stderr();
		} else {
			$ariacms->redirect("", "javascript:history.back()");
	...
	static function web_image_delete()
	{
		global $ariacms;
		$id	= @$_REQUEST["id"];
		$ariacms->delete('e4_review', 'id=' . $id);
		$ariacms->redirect("", "javascript:history.back()");
	}
}
